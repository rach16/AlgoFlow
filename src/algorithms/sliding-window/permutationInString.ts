...
export const permutationInString: Algorithm = {
  ...
  lineExplanations: {
    python: {
      ...
      9: 'get(k, 0) returns 0 for a key never seen, so its first count becomes 1 — no KeyError',
      ...
      19: 'get(k, 0) returns 0 for a key never seen, so its first count becomes 1 — no KeyError',
    ...
    java: {
      ...
      10: 'getOrDefault gives 0 instead of the null get() would return, so a first sighting becomes 1',
      ...
      23: 'getOrDefault gives 0 instead of the null get() would return, so a first sighting becomes 1',
