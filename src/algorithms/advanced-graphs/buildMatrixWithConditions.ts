...
private static int[] topo(int k, int[][] conditions) {
    ...
    return size == k ? order : null;
      ...
      lineExplanations: {
        python: {
          ...
          5: 'Read each "a before b" condition',
          6: 'a points at b',
          7: 'b gains one predecessor',
          8: 'Seed the queue with every predecessor-free value',
          9: 'Emitted ordering',
          10: "Kahn's main loop",
          11: 'Take any currently unblocked value',
          12: 'It is safe to place next',
          13: 'Relax each successor',
          14: 'One fewer predecessor blocking it',
          15: 'Fully unblocked?',
          16: 'Queue it',
          17: 'Short order means a cycle — no valid ordering',
          19: 'Row ordering',
          20: 'Column ordering',
          21: 'Either one impossible?',
          22: 'Then no matrix exists',
          24: 'Column index of every value',
          25: 'Start from an all-zero k x k grid',
          26: 'Row index = position in the row ordering',
          27: 'Drop the value at its (row, col) cell',
          28: 'Return the finished matrix',
        },
        javascript: {
          1: 'Define function with k and both condition lists',
          2: 'One helper handles rows and columns alike',
  ...
  lineExplanations: {
    python: {
      1: 'Define function with k and both condition lists',
      2: 'One helper handles rows and columns alike',
      3: 'Successor lists: a -> everything that must follow a',
      4: 'Read each "a before b" condition',
      5: 'Record the edge',
      6: '0 = unseen, 1 = on the current path, 2 = finished',
      7: 'DFS post-order accumulates here',
      9: 'Depth-first visit',
      10: 'Seeing an on-path node means a cycle',
      11: 'Cycle -> no valid ordering',
      12: 'Already finished, nothing to do',
      14: 'Mark the node as on the current path',
      15: 'Recurse into every successor first',
      17: 'Propagate the cycle failure upward',
      18: 'Node is finished',
      19: 'Append it AFTER its successors — post-order',
      22: 'Every value must be visited',
      23: 'Any cycle aborts the whole thing',
      25: 'Reversed post-order is a topological order',
      27: 'Ordering that fixes row indices',
      28: 'Ordering that fixes column indices',
      29: 'Either constraint set unsatisfiable?',
      30: 'Then return an empty matrix',
      32: 'Column index of every value',
      33: 'Start from an all-zero k x k grid',
      34: 'Row index = position in the row ordering',
      35: 'Value v lands at (rowPos, colPos) — one per row and column',
      36: 'Return the finished matrix',
    },
    javascript: {
      1: 'Define function with k and both condition lists',
      2: 'One helper handles rows and columns alike',
      ...
      26: 'Ordering that fixes row indices',
      27: 'Ordering that fixes column indices',
      28: 'Either constraint set unsatisfiable means no matrix',
      30: 'Column index of every value',
      32: 'Start from an all-zero k x k grid',
      33: 'Value v lands at (rowPos, colPos)',
      34: 'Return the finished matrix',
    },
    java: {
      1: 'Define method with k and both condition lists',
