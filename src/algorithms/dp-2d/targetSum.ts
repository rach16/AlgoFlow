...
export const targetSum: Algorithm = {
  ...
  lineExplanations: {
    python: {
      ...
      3: 'Check if subset sum is achievable (odd/negative)',
      4: 'Return 0 if no valid partition exists',
      5: 'Compute subset target: (target + total) / 2',
      6: 'Create 2D DP table filled with zeros',
      7: 'Continuation of table initialization',
      8: 'Base case: one way to make sum 0 with no items',
      9: 'Iterate over each number',
      10: 'Iterate over each possible sum value',
      11: 'Carry forward ways without current number',
      12: 'If current number fits in remaining sum',
      13: 'Add ways that include current number',
      14: 'Return count of ways to reach subset target',
    },
    javascript: {
      1: 'Define function taking nums array and target',
      2: 'Compute total sum of all numbers',
      3: 'Check if subset sum is achievable',
      4: 'Return 0 if no valid partition exists',
      5: 'Compute subset target: (target + total) / 2',
      6: 'Create 2D DP table filled with zeros',
      7: 'Continuation of array initialization',
      8: 'Continuation of fill(0)',
      9: 'Base case: one way to make sum 0 with no items',
      10: 'Iterate over each number',
      11: 'Iterate over each possible sum value',
      12: 'Carry forward ways without current number',
      13: 'If current number fits in remaining sum',
      14: 'Add ways that include current number',
      17: 'Return count of ways to reach subset target',
    },
    java: {
      1: 'Define method taking nums array and target',
      2: 'Initialize total sum variable',
      3: 'Sum all numbers in the array',
      4: 'Return 0 if partition is impossible',
      5: 'Compute subset target: (target + total) / 2',
      6: 'Create 2D DP table initialized to 0',
      7: 'Base case: one way to make sum 0 with no items',
      8: 'Iterate over each number',
      9: 'Iterate over each possible sum value',
      10: 'Carry forward ways without current number',
      11: 'If current number fits in remaining sum',
      12: 'Add ways that include current number',
      16: 'Return count of ways to reach subset target',
    },
  },
};
