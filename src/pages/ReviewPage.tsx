...
import { parseReviewId } from '../utils/reviewKinds';
...
export function ReviewPage({ onOpenAlgorithm }: ReviewPageProps) {
  ...
  const inScope = (id: string) => {
    // Everything with a kind declares whether the filter applies to it; the filter is about who
    // asks a coding question, so today nothing but a problem opts in.
    const parsed = parseReviewId(id);
    if (parsed) return !parsed.kind.audienceFiltered;
  ...
  const Row = ({ id, record }: { id: string; record: ReviewRecord }) => {
    // Anything with a kind renders from the registry; an unprefixed id is a problem, and only a
    // problem gets a clickable row, because only a problem has somewhere to open.
    const parsed = parseReviewId(id);
    if (parsed) {
      ...
      return (
        <div className="w-full flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-700/40 text-left">
          ...
          <span className="flex-1 min-w-0">
            <span className="block text-sm truncate">{kind.titleFor(itemId, { stories })}</span>
