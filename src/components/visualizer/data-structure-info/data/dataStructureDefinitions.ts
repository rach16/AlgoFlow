...
export const dataStructureDefinitions: Record<string, DataStructureDefinition> = {
  string: {
    ...
    name: 'String',
    description:
      'Operations on a WHOLE string. Strings are immutable in all three languages, which is why building one in a loop is slow and why you collect pieces then join. For single-character work see the separate char entry.',
    keyProperties: [
      'Immutable — every "modification" allocates a new string',
      'Building with += in a loop can be O(n squared); collect into a list and join once',
      'In Java use s.equals(t), never s == t — == compares references, not contents',
    ],
    methods: [
      { description: 'Length', timeComplexity: 'O(1)', python: 'len(s)', javascript: 's.length', java: 's.length()' },
      { description: 'Read the character at an index', timeComplexity: 'O(1)', python: 's[i]', javascript: 's[i]', java: 's.charAt(i)' },
      { description: 'Substring from i up to j', timeComplexity: 'O(j-i)', python: 's[i:j]', javascript: 's.slice(i, j)', java: 's.substring(i, j)' },
      { description: 'Explode into individual characters', timeComplexity: 'O(n)', python: 'list(s)', javascript: "s.split('')", java: 's.toCharArray()' },
      ...
      { description: 'Build up a string in a loop without going quadratic', timeComplexity: 'O(n)', python: "parts.append(c)  ...  ''.join(parts)", javascript: "parts.push(c)  ...  parts.join('')", java: 'StringBuilder sb;  sb.append(c);  sb.toString()' },
      { description: 'Reverse', timeComplexity: 'O(n)', python: 's[::-1]', javascript: "[...s].reverse().join('')", java: 'new StringBuilder(s).reverse().toString()' },
      { description: 'Lowercase / uppercase the whole string', timeComplexity: 'O(n)', python: 's.lower()  /  s.upper()', javascript: 's.toLowerCase()  /  s.toUpperCase()', java: 's.toLowerCase()  /  s.toUpperCase()' },
      { description: 'Sort the characters — the anagram key trick', timeComplexity: 'O(n log n)', python: "''.join(sorted(s))", javascript: "[...s].sort().join('')", java: 'char[] c = s.toCharArray(); Arrays.sort(c);' },
      ...
      { description: 'Does it start / end with something?', timeComplexity: 'O(m)', python: "s.startswith(p)  /  s.endswith(p)", javascript: 's.startsWith(p)  /  s.endsWith(p)', java: 's.startsWith(p)  /  s.endsWith(p)' },
      { description: 'Split on a delimiter', timeComplexity: 'O(n)', python: "s.split('/')", javascript: "s.split('/')", java: 's.split("/")' },
      { description: 'Strip surrounding whitespace', timeComplexity: 'O(n)', python: 's.strip()', javascript: 's.trim()', java: 's.trim()' },
      { description: 'Compare contents for equality', timeComplexity: 'O(n)', python: 'a == b', javascript: 'a === b', java: 'a.equals(b)      // NOT a == b' },
      { description: 'Is it empty?', timeComplexity: 'O(1)', python: 'not s', javascript: 's.length === 0', java: 's.isEmpty()' },
    ],
  },
  char: {
    type: 'char',
    name: 'char / Character',
    description:
      'Operations on a SINGLE character. This is a different API surface from String, and that is the part people trip on: in Java a char is a primitive, so the tests live on the static Character class, not on the char itself. Python has no char type at all — a character is just a length-1 string.',
    keyProperties: [
      'Java: char is a primitive — use Character.isDigit(c), never c.isDigit()',
      'Python: a char IS a 1-length string, so every str method works on it',
      'Subtracting the code of "a" maps a-z to 0-25, which is what int[26] counting relies on',
    ],
    methods: [
      { description: 'Is it a digit 0-9?', timeComplexity: 'O(1)', python: 'c.isdigit()', javascript: "c >= '0' && c <= '9'", java: 'Character.isDigit(c)' },
      { description: 'Is it a letter?', timeComplexity: 'O(1)', python: 'c.isalpha()', javascript: '/[a-z]/i.test(c)', java: 'Character.isLetter(c)' },
      { description: 'Is it a letter or a digit?', timeComplexity: 'O(1)', python: 'c.isalnum()', javascript: '/[a-z0-9]/i.test(c)', java: 'Character.isLetterOrDigit(c)' },
      { description: 'Is it whitespace?', timeComplexity: 'O(1)', python: 'c.isspace()', javascript: '/\\s/.test(c)', java: 'Character.isWhitespace(c)' },
      { description: 'Is it uppercase / lowercase?', timeComplexity: 'O(1)', python: 'c.isupper()  /  c.islower()', javascript: 'c === c.toUpperCase()', java: 'Character.isUpperCase(c)  /  isLowerCase(c)' },
      { description: 'Lowercase / uppercase a single char', timeComplexity: 'O(1)', python: 'c.lower()  /  c.upper()', javascript: 'c.toLowerCase()  /  c.toUpperCase()', java: 'Character.toLowerCase(c)  /  toUpperCase(c)' },
      ...
      { description: 'Character to its numeric code point', timeComplexity: 'O(1)', python: 'ord(c)', javascript: 'c.charCodeAt(0)', java: '(int) c' },
      { description: 'Code point back to a character', timeComplexity: 'O(1)', python: 'chr(n)', javascript: 'String.fromCharCode(n)', java: '(char) n' },
      { description: 'Digit character to the number it represents', timeComplexity: 'O(1)', python: 'int(c)', javascript: 'Number(c)', java: "c - '0'      // or Character.getNumericValue(c)" },
      { description: 'A number 0-9 to its digit character', timeComplexity: 'O(1)', python: 'str(d)', javascript: 'String(d)', java: "(char)(d + '0')" },
      { description: 'Compare two characters', timeComplexity: 'O(1)', python: 'a == b', javascript: 'a === b', java: 'a == b      // fine here, char is primitive' },
    ],
  },
  array: {
    type: 'array',
